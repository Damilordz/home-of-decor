...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const productSections = document.querySelectorAll(".trending-products-class-all");
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const commentLinkSubImages = document.querySelectorAll('.comment-link-sub img');
  
  commentLinkSubImages.forEach((img) => {
      img.addEventListener('click', () => {
          // Get the corresponding java-hide section
          const javaHideSection = img.closest('.comment-link-sub').nextElementSibling;
...
document.querySelectorAll('.image-content-1').forEach(item => {
  ...
  const cancelImages = document.querySelectorAll('.client-comment--display .comment-display img');
